...
SoundService::SoundService(android_app* pApplication) :
        mApplication(pApplication), mEngineObj(NULL), mEngine(NULL), mOutputMixObj(
                ...
                NULL), mPlayerObj(NULL), mPlayer(NULL), mPlayerQueue(NULL), mSounds(), mSoundCount(
                0), mRecorderObj(NULL), mRecorder(NULL), mRecorderQueue(NULL), mRecordSize(
                44100 * 2), mRecordBuffer(new int16_t[mRecordSize]) {
...
status SoundService::start() {
    ...
    ERROR: Log::error("Error while starting SoundService");
    stop();
    return STATUS_KO;
}
...
status SoundService::startSoundPlayer() {
    ...
    ERROR: Log::error("Error while starting SoundPlayer");
    return STATUS_KO;
}
...
status SoundService::startSoundRecorder() {
    ...
    const SLInterfaceID lSoundRecorderIIDs[] = { SL_IID_RECORD,
            SL_IID_ANDROIDSIMPLEBUFFERQUEUE };
    const SLboolean lSoundRecorderReqs[] = { SL_BOOLEAN_TRUE, SL_BOOLEAN_TRUE };

    lRes = (*mEngine)->CreateAudioRecorder(mEngine, &mRecorderObj, &lDataSource,
            &lDataSink, lSoundRecorderIIDCount, lSoundRecorderIIDs,
            lSoundRecorderReqs);
    if (lRes != SL_RESULT_SUCCESS )
        goto ERROR;
    ...
    lRes = (*mRecorderObj)->GetInterface(mRecorderObj, SL_IID_RECORD,
            &mRecorder);
    if (lRes != SL_RESULT_SUCCESS )
        goto ERROR;
...
status SoundService::playBGM(const char* pPath) {
    ...
    lRes = (*mEngine)->CreateAudioPlayer(mEngine, &mBGMPlayerObj, &lDataSource,
...
void SoundService::playSound(Sound* pSound) {
    ...
    ERROR: Log::error("Error trying to play sound");
}
...
void SoundService::recordSound() {
    ...
    if (lRecorderState == SL_OBJECT_STATE_REALIZED ) {
        // Stops any current recording.
        lRes = (*mRecorder)->SetRecordState(mRecorder, SL_RECORDSTATE_STOPPED );
        if (lRes != SL_RESULT_SUCCESS )
            goto ERROR;
        ...
        lRes = (*mRecorderQueue)->Enqueue(mRecorderQueue, mRecordBuffer,
    ...
    ERROR: Log::error("Error trying to record sound");
}
...
void SoundService::playRecordedSound() {
    ...
    if (lRecorderState == SL_OBJECT_STATE_REALIZED ) {
        ...
        // Plays the new sound.
        lRes = (*mPlayerQueue)->Enqueue(mPlayerQueue, mRecordBuffer,
                mRecordSize * sizeof(int16_t));
        if (lRes != SL_RESULT_SUCCESS )
            goto ERROR;
    }
    ...
    ERROR: Log::error("Error trying to play recorded sound");
}

void SoundService::callback_recorder(SLAndroidSimpleBufferQueueItf pQueue,
        void* pContext) {
    SLresult lRes;
    SoundService& lService = *(SoundService*) pContext;

    Log::info("Ended recording sound.");
    lRes = (*(lService.mRecorder))->SetRecordState(lService.mRecorder,
